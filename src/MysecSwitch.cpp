...
void MysecSwitch::processaUdp() {
  // Agora processa UDP.
  if (mysecUdpNet != NULL && mysecUdpNet->isConfigured() && !mysecUdpNet->makeSharedKey(_mysecDeviceState.passkey1)) {
...
void MysecSwitch::loop() {
  ...
  for (int index = 0; index < _mysecDeviceState.numPins; index++) {
    ...
    if (
      ...
      _mysecDeviceState.applyNext(index, (mysecUdpNet != NULL && mysecUdpNet->isDesabilitaAutomatico()));
    }
  }
  // se temos chave nova então processamos
...
//          MYSECSWITCH_DEBUGLN(F("Switch loop Sincronizando com servidor e viewer"));
          ...
          if (resultado) {
            ...
            _mysecDeviceState.state = MysecDeviceState::STATE_IDLE;
            _mysecDeviceState.mysecParser->decodeResponse(msgid, response, m, mysecUdpNet != NULL && mysecUdpNet->isDesabilitaAutomatico());
          }
        }
      }
    }
